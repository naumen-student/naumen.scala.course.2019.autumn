object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, summary: (Vector2D, Vector2D) => Double,  rightVec0: Vector2D, rightVec1: Vector2D) = summary(leftVec0, leftVec1) +summary(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val weight = (ball : (Int, Double)) => 4 / 3 * Math.PI * Math.pow(tuple._1, 3) * ball._2
