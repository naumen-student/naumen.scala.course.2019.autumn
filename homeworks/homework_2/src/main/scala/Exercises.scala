...
object Exercises {
    ...
    {
        var divNumber = 2;
        var listOfMultipliers = new ArrayBuffer[Int]();
        var numbr = number;
        var counter = 1;
        while (numbr > 1)
        {
            if(numbr % divNumber == 0)
                {
                    numbr /= divNumber;
                    if(counter == 1)
                    {
                        listOfMultipliers += divNumber;
                        counter += 1;
                    }
                }
            else
    ...
    */
    ...
    {
        def volume(radius: Int) = 4 / 3 * Math.PI * Math.pow(radius, 3);
        def weight(radius: Int, density: Double) = volume(radius) * density;
        ballsArray.toSeq.sortBy(elem => weight(elem._2._1, elem._2._2)).map(elem => elem._1);
    }
