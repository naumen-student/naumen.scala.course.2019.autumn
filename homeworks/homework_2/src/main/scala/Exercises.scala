...
object Exercises {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        var unsortedBalls = ballsArray.keys.toBuffer

        while(unsortedBalls.nonEmpty){
            var lightWeight = Double.MaxValue
            var lightBallName = ""

            for(ball <- unsortedBalls){
                val currentWeight = calculateWeight(ballsArray(ball)._1,
                                    ballsArray(ball)._2)
                if(currentWeight < lightWeight) {
                    lightBallName = ball
                    lightWeight = currentWeight
                }
            }
            unsortedBalls -= lightBallName
            result += lightBallName
        }
        result
    }
