...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    var numberBuf = number

    for (i <- 2 to number) {
      if (numberBuf % i == 0 && numberBuf > 1) {
        factors += i;
        while (numberBuf % i == 0)
          numberBuf /= i
      }
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
                ...
                rightVec0: Vector2D, rightVec1: Vector2D) = function(leftVec0, leftVec1) + function(rightVec0, rightVec1)
  ...
  */
  ...
  /*def findBallMass(radius: Int, density: Double): Double = {
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    val ballMass = (ball:(Int, Double)) => 4 / 3 * java.lang.Math.PI * java.lang.Math.pow(ball._1, 3) * ball._2

    ballsArray.mapValues(ballMass).toSeq.sortBy(_._2).map(_._1)
  }
