...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        for (i <- 2 to number / 2; if chNumber > 1) {
            if (chNumber % i == 0) {
                resList += i
                while (chNumber % i == 0)
                    chNumber /= i
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        {
            def WeightCurrentBall = (tuple:(Int,Double)) => 4 / 3 * Math.pow(tuple._1, 3) * tuple._2
            ballsArray.mapValues(WeightCurrentBall).toSeq.sortBy(_._2).map(_._1)
        }
