object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    var check = number
    val resList = scala.collection.mutable.ListBuffer.empty[Int]
    for {i <- 2 to number
         if check > 1
         if check % i == 0
    } {
      resList += i
      while (check % i == 0) {check /= i}
    }
    resList
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    def relativeMass (ball: (Int, Double)): Double = {
      // ball = (radius, density)
      // missed the multiplier (4.0 / 3.0 * java.lang.Math.PI) because it's the same for everyone
      ball._2 * java.lang.Math.pow(ball._1,3)
    }

    ballsArray.mapValues(relativeMass).toSeq.sortBy(_._2).map(_._1)
  }
