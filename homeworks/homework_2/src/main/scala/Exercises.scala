object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val listOfSimpleDivisors =scala.collection.mutable.MutableList[Int]();
        var keepNumber = number
        var check = 0
        var divider = 2
        while (keepNumber>1 && divider <= number/2 ){
            if(keepNumber % divider == 0) {
                keepNumber /= divider
                if(check != divider){
                    check = divider
                    listOfSimpleDivisors += check
                }
            }
            else if(divider == 2){
                divider += 1
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val weight =(radius: Int, density: Double)  => 4 / 3 * Math.PI * Math.pow(radius, 3) * density
        def ballWeight (ball: (Int, Double)) : Double = {
            val (rad, den) = ball
            weight(rad, den)}
