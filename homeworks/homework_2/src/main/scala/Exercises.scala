...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val primeNumbers = mutable.MutableList[Int]()
        var copyNumber = number
        var multiplier = 2

        while (multiplier * multiplier <= copyNumber) {
            if (copyNumber % multiplier == 0) {
                primeNumbers += multiplier
                copyNumber /= multiplier
            }
            else {
                multiplier += 1
            }
        ...
        if (copyNumber > 1) {
            primeNumbers += copyNumber
        }
        val result = primeNumbers.distinct.toSeq
        result
    }
