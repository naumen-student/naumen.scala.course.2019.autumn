object Exercises {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def mass (tuple: (Int, Double)): Double = {
              val (r,d)=tuple
              d*(4.0/3)*java.lang.Math.PI*java.lang.Math.pow(r,3)
          }
        def getMass(tuple: (String, Double)): Double = {
            val (r,d)=tuple
            d
        }
        ballsArray.mapValues(mass).toSeq.sortBy(getMass).unzip._1
    }
