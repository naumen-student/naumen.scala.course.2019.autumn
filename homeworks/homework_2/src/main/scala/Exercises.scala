...
object Exercises {
  ...
  def primeFactor(number: Int): ListBuffer[Int] = {
    var vs = 2
    var num = number
    val seq = ListBuffer.empty[Int]
    while (num != 1) {
      if (num % vs == 0) {
        if (seq.contains(vs) == false) {
          seq += vs
          num = num / vs
        }
        else {
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): ListBuffer[String] = {
    val res = ListBuffer.empty[String]
    var balls = ballsArray.keys.toBuffer

    while (balls.nonEmpty) {
      var minBall = ""
      var minWeight = Double.MaxValue
      for (ball <- balls) {
        val currentWeight = (4 / 3 * math.Pi * math.pow(ballsArray(ball)._1, 3) * ballsArray(ball)._2)
