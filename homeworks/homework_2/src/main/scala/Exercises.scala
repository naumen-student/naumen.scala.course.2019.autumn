...
//   Масса=плотность*объем=плотность*4/3*Пи*радиус в кубе()
def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    //var a = Seq()

    def mass (information: (Int, Double)): Double=
    {
        val (radius,density)=information
        density*(4.0/3)*java.lang.Math.PI*java.lang.Math.pow(radius,3)
    }

    def getMass(information: (String, Double)): Double = {
        val (radius,density)=information
        density
    }

    ballsArray.mapValues(mass).toSeq.sortBy(getMass).map(_._1)
}