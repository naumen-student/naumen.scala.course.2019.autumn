...
object Exercises {
    ...
    def prettyBooleanFormatter2(x: Any): String = {
        x match {
            case bool: Boolean => booleanFormatter(bool)
            case any => any.toString
        }
    }

    def prettyBooleanFormatter3(x: Any): String = try  {
         booleanFormatter(x.asInstanceOf[Boolean])
     }
   catch {
     case _ => x.toString;
     ...
     def booleanFormatter(x: Boolean): String = {
     ...
     */ 
    def sum1(x: Int, y: Int): Int = sumIntegers(Point(x, y))
