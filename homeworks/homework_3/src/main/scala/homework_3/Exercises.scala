...
object Exercises {
     ...
     */
    def FormatBool(x: Boolean) : String = {
        if (x) {
            "правда"
        }
        else {
    ...
    def prettyBooleanFormatter1(x: Any): String = {
        x match {
            case bool: Boolean => FormatBool(bool)
    ...
    def prettyBooleanFormatter2(x: Any): String = {
        try  {
            FormatBool(x.asInstanceOf[Boolean])
        }
        catch {
            case _ => x.toString;
        }
    ...
    def prettyBooleanFormatter3(x: Any): String = {
        if (x.getClass.getSimpleName.equals("Boolean")) {
            booleanFormatter(x.asInstanceOf[Boolean])
        } else {
            x.toString
        }
    }
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(Two(x, y))

    case class Two(x: Int, y: Int)  extends Iterable[Int] {
